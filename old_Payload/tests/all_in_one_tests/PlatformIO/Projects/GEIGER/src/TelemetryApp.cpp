...
static const uint8_t BMP388_ADDR = I2C_ADDR_BMP388;
#endif

#if Mag_Sensor
static const uint8_t BMM350_ADDR = I2C_ADDR_BMM350;
...
void TelemetryApp::begin() {
  ...
  // Drive all SPI CS pins HIGH before any bus transactions.
  // The ICM-45686 has an internal CS pull-down; if GPIO 16 floats as an input
  // the IMU will be selected and corrupt SD initialisation.
#if IMU_Sensor
  ...
  #if Baro_Sensor
    baroOk = bmp.begin_I2C(BMP388_ADDR, &Wire);
    if (baroOk) {
      bmp.setTemperatureOversampling(BMP3_OVERSAMPLING_8X);
      bmp.setPressureOversampling(BMP3_OVERSAMPLING_4X);
      bmp.setIIRFilterCoeff(BMP3_IIR_FILTER_COEFF_3);
      #if WrightToSerial
        Serial.println("[BARO] BMP388 OK");
      #endif
    } else {
      #if WrightToSerial
        Serial.println("[BARO] BMP388 init failed (check addr 0x77/0x76)");
      #endif
    }
  #endif
...
void TelemetryApp::tick() {
  ...
  #if LoRa
    if (loraOk_) {
      uint8_t mavBuf[MAVLINK_FRAME_BUF_SIZE];
      size_t frameLen = buildMavlinkFrame(mavBuf, geigerCount, now);
      auto r = lora_.transmitWithRetry(mavBuf, frameLen, loRaRetries_);
      loraStatus = r.status;
...
#if LoRa
size_t TelemetryApp::buildMavlinkFrame(uint8_t* buf, int geigerCount, uint32_t tMs) {
  mavlink_message_t msg;
  size_t offset = 0;

  // HEARTBEAT — identifies this node as COSMIC_RADIATION+ (sysid=42)
  mavlink_msg_heartbeat_pack(MAVLINK_SYSID_COSMIC, MAVLINK_COMPID_COSMIC, &msg,
    MAV_TYPE_GENERIC, MAV_AUTOPILOT_INVALID, 0, 0, MAV_STATE_ACTIVE);
  offset += mavlink_msg_to_send_buffer(buf + offset, &msg);

  // Geiger counts per 1-second interval
  mavlink_msg_named_value_int_pack(MAVLINK_SYSID_COSMIC, MAVLINK_COMPID_COSMIC, &msg,
    tMs, "GEIGER_C1S", static_cast<int32_t>(geigerCount));
  offset += mavlink_msg_to_send_buffer(buf + offset, &msg);

  // IMU + magnetometer (gyro converted mdps→mrad/s, mag uT*10→mgauss maps 1:1)
  auto mdps_to_mrad = [](int16_t mdps) -> int16_t {
    return clamp16(static_cast<int32_t>(mdps) * 17453 / 1000000);
  };
  mavlink_msg_scaled_imu_pack(MAVLINK_SYSID_COSMIC, MAVLINK_COMPID_COSMIC, &msg,
    tMs,
    lastSens_.ax_mg,  lastSens_.ay_mg,  lastSens_.az_mg,
    mdps_to_mrad(lastSens_.gx_mdps), mdps_to_mrad(lastSens_.gy_mdps), mdps_to_mrad(lastSens_.gz_mdps),
    lastSens_.mx_uT10, lastSens_.my_uT10, lastSens_.mz_uT10,
    lastSens_.t_cC);
  offset += mavlink_msg_to_send_buffer(buf + offset, &msg);

  // Barometer: Pa→hPa for press_abs, temperature already in cdegC
  mavlink_msg_scaled_pressure_pack(MAVLINK_SYSID_COSMIC, MAVLINK_COMPID_COSMIC, &msg,
    tMs,
    static_cast<float>(lastSens_.p_Pa) / 100.0f, 0.0f,
    lastSens_.t_cC, 0);
  offset += mavlink_msg_to_send_buffer(buf + offset, &msg);
